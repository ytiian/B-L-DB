...
struct TableBuilder::Rep {
  Rep(const Options& opt, WritableFile* f, const bool is_m)
      : options(opt),
        ...
        data_block(is_m ? static_cast<BlockBuilder*>(new FlatBlockBuilder(&options))
                             : static_cast<BlockBuilder*>(new PrefixBlockBuilder(&options))),
        index_block(new PrefixBlockBuilder(&index_block_options)),
        data_info_block(is_m ? static_cast<BlockBuilder*>(new FlatBlockBuilder(&options))
                             : nullptr),
        model_info_block(is_m ? static_cast<BlockBuilder*>(new FlatBlockBuilder(&options))
                             : nullptr),
  ...
  ~Rep() {
    delete data_block;
    delete index_block;
    if(data_info_block != nullptr) delete data_info_block;
    if(model_info_block != nullptr) delete model_info_block;
  }
  ...
  IndexHandle pending_data_info_handle;  // Handle to add to index info block

  std::string compressed_output;
};

TableBuilder::TableBuilder(const Options& options, WritableFile* file, const bool is_model)
    : rep_(new Rep(options, file, is_model)) {
...
void TableBuilder::Add(const Slice& key, const Slice& value) {//插入kv对
  ...
  if(r->is_model){
    PushPendingKeys(Slice(key.data(), key.size()-8).ToString(), entry_count_++);
  }
  //std::cout<<"Push pending key: "<<key.ToString()<<" with value: "<<count<<std::endl;
...
void TableBuilder::AddToModelInfo(){
...
Status TableBuilder::Finish() {
  ...
  models_result_ = models_algorithm_.build(r->options.error_bound); //error_bound=20

  // auto end = std::chrono::high_resolution_clock::now();
  // auto duration_us = std::chrono::duration_cast<std::chrono::microseconds>(end - start).count();

  // std::cout << "build() cost: " << duration_us/1000 << " ms" << std::endl;

  BlockHandle filter_block_handle, metaindex_block_handle, index_block_handle, 
            data_info_block_handle, model_info_block_handle;
  ...
  if(r->data_info_block != nullptr){
      WriteRawBlock(r->data_info_block->Finish(),kNoCompression,&data_info_block_handle);
  }

  if(r->model_info_block != nullptr){

      AddToModelInfo();

      WriteRawBlock(r->model_info_block->Finish(),kNoCompression,&model_info_block_handle);
  }

  // Write metaindex block
  if (ok()) {
    PrefixBlockBuilder meta_index_block(&r->options);

    if(r->is_model){
        std::string prefix;
        PutFixed64(&prefix, models_result_.common_prefix_len);
        std::string error_bound;
        PutFixed32(&error_bound, models_result_.error_bound);
        
        meta_index_block.Add("common_prefix_len", prefix);

        std::string key = "data_info";
        std::string handle_encoding;
        data_info_block_handle.EncodeTo(&handle_encoding);
        meta_index_block.Add(key, handle_encoding);

        meta_index_block.Add("error_bound", error_bound);
    }    
    ...
    if(r->is_model){
        std::string key = "model_info";
        std::string handle_encoding;
        model_info_block_handle.EncodeTo(&handle_encoding);
        // Append common_prefix_len to the handle or key? 
        // The user asked to persist ALL content of PLRResult.
        // We can append it to the handle encoding or add another meta entry.
        // Let's append it to the handle encoding as varint.
        meta_index_block.Add(key, handle_encoding);
    }
