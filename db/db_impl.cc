...
//记录一次Compaction的相关信息：
//输出文件，Compaction类对象（包含输入文件），最小快照
...
//新建一个数据库
//主要功能：初始化一个VersionEdit，
//写入一个新建的manifest，最后修改CURRENT
Status DBImpl::NewDB() {
  VersionEdit new_db;
  ...
  {
    //在manifest文件中记录初始化的这个VersionEdit
    log::Writer log(file);
    std::string record;
...
Status DBImpl::Recover(VersionEdit* edit, bool* save_manifest) {
  ...
  //文件dbname/CURRENT不存在，说明数据库原来不存在
  if (!env_->FileExists(CurrentFileName(dbname_))) {
    if (options_.create_if_missing) {//不存在就创建新的
    ...
    if (options_.error_if_exists) {//存在就是出错
  ...
  //expected保存的是记录在versionSet中的文件
  //从expected中不断踢掉实际有存在（在磁盘上读到的）文件
  //所以如果expected最终不为空
  //说明存在VersionSet记录的，可磁盘上却不存在的
  //也就是有文件丢失
  if (!expected.empty()) {
    char buf[50];
...
Status DBImpl::RecoverLogFile(uint64_t log_number, bool last_log,
  ...
  //写WAL的时候是以WriteBatch的单位，
  //因此读一条记录也是一个WriteBatch
  ...
  //是否重用最后一个log
  //？为什么可以重用最后一个log：可能没写满？
  //？为什么要compaction==0：这个日志没能够写满一个mem，因此mem可以一起被重用
  if (status.ok() && options_.reuse_logs && last_log && compactions == 0) {
    ...
    if (env_->GetFileSize(fname, &lfile_size).ok() &&
      ...
      if (mem != nullptr) {
        //mem_是DBImpl记录的当前的active mem
        mem_ = mem;//log被重用的同时，mem也被重用
        mem = nullptr;
      } else {
  ...
  //最后一块不为空的mem也要flush
  if (mem != nullptr) {//为空说明被重用，不为空说明没有被重用，flush
...
//非recover过程中发生的 mem flush
//调用：都是先判断imm_不等于null，然后对imm_生成新sstable
void DBImpl::CompactMemTable() {
  ...
  if (s.ok()) {
    ...
    //每次compaction/flush都会生成新edit，要及时应用到Version上，并记录在manifest中
...
Status DBImpl::OpenCompactionOutputFile(CompactionState* compact) {
  ...
  if (s.ok()) {
    compact->builder = new TableBuilder(options_, compact->outfile, true);
  }
  return s;
}
...
Status DBImpl::DoCompactionWork(CompactionState* compact) {
  ...
  assert(compact->builder == nullptr);
  assert(compact->outfile == nullptr);
  if (snapshots_.empty()) {//不使用快照，只考虑当下的seq
  ...
  while (input->Valid() && !shutting_down_.load(std::memory_order_acquire)) {
    ...
    if (has_imm_.load(std::memory_order_relaxed)) {
      ...
      if (imm_ != nullptr) {
        CompactMemTable();
        //就是因为有了这个，会暂时对下当前的多文件合并，转去刷immemtable
        //而CompactMemTable会调用DeleteObsoleteFiles主要为了删没用日志文件
        //为了避免误删掉刚刚多文件合并产生的新文件，
        //需要将产生的新文件用OpenCompactionOutputFile
        //函数加入到pending_outputs_，等合并完成并应用到新版本，再调用
        //CleanupCompaction清除刚加入到pending_outputs_的文件编号
    ...
    if (!ParseInternalKey(key, &ikey)) {
      ...
      if (!has_current_user_key ||
        ...
        last_sequence_for_key = kMaxSequenceNumber;
        //因为第一次出现的user_key不允许删除，
        //所有将last_sequence_for_key设为最大值
      }

      //要确保保留了小于快照版本的seq最大的一个key
      if (last_sequence_for_key <= compact->smallest_snapshot) {
        //当上一个key的seq<=最小快照号，本个key（userkey和上个相同，seq更小）才能被删除
...
Iterator* DBImpl::NewInternalIterator(const ReadOptions& options,
  ...
  *latest_snapshot = versions_->LastSequence();
  ...
  Iterator* disk_iter = 
      NewDiskIterator(internal_comparator_.user_comparator(), &list[0], list.size(), sindex_, index_map);
...
Status DBImpl::Get(const ReadOptions& options, const Slice& key,
  ...
  if (options.snapshot != nullptr) {//读操作时指定了快照
    snapshot =
